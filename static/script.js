...
async function carregarDados(isFallback = false) {
    ...
    try {
        ...
        if (isFallback) {
            ...
            if (dados.erro) {
                exibirBannerErro(dados.mensagem || "Erro na API.");
            } else {
                fecharBannerErro();
            }
        }
        ...
        exibirBannerErro("Conexão falhou. Exibindo dados de demonstração.");
...
function mostrarSkeleton(visivel) {
    document.getElementById("skeletonLoader").style.display = visivel ? "block" : "none";
    document.getElementById("dashboardReal").style.display = visivel ? "none" : "block";
}
...
function atualizarHeaderStatus(dados) {
    ...
    document.getElementById("footerUpdate").textContent = `${dados.summary?.total || 0} nodes monitorados`;
}
...
function atualizarTabela(dados) {
    ...
    const servicos = dados.services || dados.nodes || [];
    ...
    for (const servico of servicos) {
        ...
        const latencia = parseFloat(servico.latency) || 0;
        const latClass = latencia < 80 ? "latency-low" : (latencia < 200 ? "latency-mid" : "latency-high");
        const uptime = parseFloat(servico.uptime) || 0;
        ...
        html += `
            <div class="node-row">
                ...
                <div class="node-value">
                    <div class="progress-bar"><div class="progress-fill progress-fill-uptime" style="width:${uptime}%"></div></div>
...
function normalizarStatus(status) {
    ...
    const inst = ["degraded_performance","degraded","degradado","instavel","unstable","partial_outage","warning"];
...
function renderizarCharts(dados) {
        ...
        chartInstances.status = new Chart(ctx1, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderColor: Array(3).fill(isLight ? "#ffffff" : "#15121A"),
    ...
    if (ctx2 && servicos.length > 0) {
        ...
        const data = servicos.map(s => parseFloat(s.latency) ? Math.round(s.latency*10)/10 : 0);
    ...
    if (ctx3 && servicos.length > 0) {
        const labels = servicos.map(s => s.name || "?");
        const data = servicos.map(s => parseFloat(s.uptime) ? Math.round(s.uptime*100)/100 : 0);
        ...
        chartInstances.uptime = new Chart(ctx3, {
            ...
            options: {
                ...
                scales: { 
                    ...
                    y: { grid: { display: false }, beginAtZero: true, max: 100, ticks: { font: { size: 10 }, callback: v => v + "%" } } 
                }
            }
        });
    }
...
function detectarAlertas(dados) {
    ...
    for (const servico of servicos) {
        ...
        // Node offline
        if (!ops.includes(statusRaw)) {
            if (!alertasAtivos.some(a => a.node === nome && a.tipo === "offline" && !a.resolvido)) {
                novosAlertas.push({
                    id: Date.now() + Math.random(),
                    tipo: "offline", severidade: "critical",
                    titulo: `${nome} está offline`,
                    descricao: `O node ${nome} não está respondendo às requisições.`,
                    node: nome, hora: agora, resolvido: false, visto: false
                });
        ...
        if (latencia > 200) {
            if (!alertasAtivos.some(a => a.node === nome && a.tipo === "latencia" && !a.resolvido)) {
                novosAlertas.push({
                    id: Date.now() + Math.random(),
                    tipo: "latencia", severidade: "warning",
        ...
        // Latência moderada (100-200ms)
        if (latencia > 100 && latencia <= 200) {
            if (!alertasAtivos.some(a => a.node === nome && a.tipo === "latencia-media" && !a.resolvido)) {
                novosAlertas.push({
                    id: Date.now() + Math.random(),
                    tipo: "latencia-media", severidade: "info",
    ...
    for (const alerta of alertasAtivos) {
        ...
        const latNode = parseFloat(node.latency) || 0;

        if (alerta.tipo === "offline" && ops.includes(statusNode)) {
            alerta.resolvido = true;
...
function renderizarAlertas() {
    ...
    if (ativos.length > 0) {
        ...
        for (const a of ativos) {
            ...
            const borderColor = a.severidade === 'critical' ? 'rgba(251,113,133,0.15)' : a.severidade === 'warning' ? 'rgba(250,204,21,0.15)' : 'rgba(96,165,250,0.15)';
            const svgIcon = a.severidade === "critical"
                ? `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`
                : `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>`;

            html += `
                <div class="alert-item" style="border-color:${borderColor}">
                    <div class="alert-icon ${iconClass}">${svgIcon}</div>
                    <div class="alert-body">
                        <div class="alert-title">${a.titulo}</div>
                        <div class="alert-desc">${a.descricao}</div>
                        <div class="alert-time">${a.hora.toLocaleTimeString("pt-BR")}</div>
                    </div>
...
function renderizarLogs() {
    ...
    for (const log of logsRegistros.slice(0, 50)) {
        html += `
            <div class="log-entry">
                <span class="log-time">${log.hora.toLocaleTimeString("pt-BR")}</span>
                <span class="log-dot ${dotClass[log.tipo] || "log-dot-info"}"></span>
...
function marcarAlertasVistos() {
    alertasAtivos.forEach(a => { if (!a.resolvido) a.visto = true; });
    const badge = document.getElementById("alertBadge");
...
document.addEventListener("DOMContentLoaded", function () {
    console.log("⚡ JD Node & Infrastructure Monitoring rodando");
